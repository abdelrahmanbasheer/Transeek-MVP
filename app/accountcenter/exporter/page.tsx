...
const AccExpo = () => {
    ...
    useEffect(() => {
      const fetchWarehouses = async () => {
          ...
          const response = await fetch(`http://localhost:3000/api/exporters/${user?.username}`);
          const data = await response.json();
          setAllShipments(data);
      }
      const fetchShipments = async () => {
      
        const response = await fetch('http://localhost:3000/api/shipments/2');
        const data = await response.json();
        setShipments(data);
    }
    fetchShipments();
        ...
        <div className='flex m-[50px] h-screen gap-16'>
            <div className="flex flex-col items-center ">
             <img className='' src={AcenterPlaceholder.src} alt="" />
                <h1 className='text-4xl font-bold mt-6 '>{allShipments?.username}</h1>
